...
const NametagBinding_js_1 = require("../nametag/NametagBinding.js");
...
class NostrClient {
    ...
    handleEventMessage(json) {
        ...
        if (!subscription)
            return;
        try {
            const event = Event_js_1.Event.fromJSON(eventData);
    ...
    handleAuthMessage(relayUrl, json) {
        ...
        const authEvent = Event_js_1.Event.create(this.keyManager, {
    ...
    async publishEncryptedMessage(recipientPubkeyHex, message) {
        ...
        const event = Event_js_1.Event.create(this.keyManager, {
    ...
    async sendTokenTransfer(recipientPubkeyHex, tokenJson, options) {
        const TokenTransferProtocol = await Promise.resolve().then(() => __importStar(require('../token/TokenTransferProtocol.js')));
        const event = await TokenTransferProtocol.createTokenTransferEvent(this.keyManager, recipientPubkeyHex, tokenJson, options);
        return this.publishEvent(event);
    }
    /**
    ...
    async sendPaymentRequest(targetPubkeyHex, request) {
        const PaymentRequestProtocol = await Promise.resolve().then(() => __importStar(require('../payment/PaymentRequestProtocol.js')));
        const event = await PaymentRequestProtocol.createPaymentRequestEvent(this.keyManager, targetPubkeyHex, request);
        return this.publishEvent(event);
    }
    /**
    ...
    async sendPaymentRequestResponse(targetPubkeyHex, response) {
        const PaymentRequestProtocol = await Promise.resolve().then(() => __importStar(require('../payment/PaymentRequestProtocol.js')));
        const event = await PaymentRequestProtocol.createPaymentRequestResponseEvent(this.keyManager, targetPubkeyHex, {
    ...
    async publishNametagBinding(nametagId, unicityAddress, identity) {
        ...
        const event = await (0, NametagBinding_js_1.createBindingEvent)(this.keyManager, nametagId, unicityAddress, undefined, identity);
        try {
    ...
    async publishIdentityBinding(identity) {
        const event = (0, NametagBinding_js_1.createIdentityBindingEvent)(this.keyManager, identity);
        try {
            await this.publishEvent(event);
            return true;
        }
    ...
    async queryPubkeyByNametag(nametagId) {
        return this.queryWithFirstSeenWins((0, NametagBinding_js_1.createNametagToPubkeyFilter)(nametagId), (event) => event.pubkey);
    }
    /**
    ...
    async queryBindingByNametag(nametagId) {
        return this.queryWithFirstSeenWins((0, NametagBinding_js_1.createNametagToPubkeyFilter)(nametagId), NametagBinding_js_1.parseBindingInfo);
    }
    /**
    ...
    async queryBindingByAddress(address) {
        return this.queryWithFirstSeenWins((0, NametagBinding_js_1.createAddressToBindingFilter)(address), NametagBinding_js_1.parseBindingInfo);
    }
    /**
